  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-bold text-white mb-2">My Work Logs</h1>
          <p className="text-slate-400">Track your submitted work</p>
        </div>
      ...
      <div className="space-y-4">
        {logs.length > 0 ? (
          logs.map((log) => (
            <div key={log.id} className="bg-slate-800 border border-slate-700 rounded-xl p-6">
              ...
              {log.total_hours && (
                <p className="text-slate-400 text-sm mb-2">⏱️ {log.total_hours} hours</p>
              )}
              <Link href={`/interns/workspace/work-logs/${log.id}`}>
                <button className="text-emerald-400 hover:text-emerald-300 transition text-sm">
                  View Details →
                </button>
              </Link>
            </div>
