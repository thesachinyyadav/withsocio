...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (sentClient?.list) {
      const result = await sentClient.list(params);

      if (result?.error) {
        return NextResponse.json({ error: result.error.message || "Failed to list sent emails" }, { status: 500 });
      }
      ...
      const fallbackResponse = await fetch(`https://api.resend.com/emails?${query.toString()}`, {
        method: "GET",
        headers: {
      ...
      if (!fallbackResponse.ok) {
        return NextResponse.json(
          { error: fallbackPayload?.message || fallbackPayload?.error || "Failed to list sent emails" },
    ...
    console.error("Mailbox sent list error:", error);
    return NextResponse.json({ error: "Failed to list sent emails" }, { status: 500 });
  }
}
